...
DEFAULT_DATA    = "assets/data/cloudflare-edge-locations.json"
...
_log_lines: list[str] = []
_log_file: Path | None = None  # set once main() knows the run's timestamp
...
def write_log() -> None:
    # Called once, from main()'s finally block, so the log is always written --
    # success, a handled fatal error, or an uncaught exception.
    log_file = _log_file or (LOG_DIR / "sync-cloudflare-pops-early-failure.log")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=f"Path to cloudflare-edge-locations.json (default: {DEFAULT_DATA})",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    _log_file = LOG_DIR / f"{run_started.strftime('%Y-%m-%d_%H-%M-%S')}.log"

    sep  = "=" * 70
    thin = "-" * 70

    log(sep)
    log(f"  sync-cloudflare-pops.py — {run_started.strftime('%Y-%m-%d %H:%M:%S UTC')}")
    ...
    try:
        ...
        log(f"[fatal] Could not fetch Cloudflare locations: {exc}")
        raise SystemExit(1)
    ...
    if not edges_json.exists():
        log(f"[fatal] '{edges_json}' not found.")
        raise SystemExit(1)
