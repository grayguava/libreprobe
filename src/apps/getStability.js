...
import { interpret }         from "../measurement/rtt/interpret.js";
import { populateProviders, getSelectedProvider as getPickerValue, disablePicker, enablePicker } from "../ui/providerPicker.js";
...
async function loadProviders() {
  try {
    const res = await fetch("/assets/data/providers.json");
...
function getSelectedProvider() {
  const selected = getPickerValue();
  return providers.find(p => p.id === (selected || selectedProviderId)) ?? providers[0] ?? null;
}
...
export async function runHealthTest({
  ...
  result.handshake = await measureHandshake(endpoint);
...
export function init() {
  ...
  const torBanner = document.getElementById("tor-banner");
  getConnectionInfo().then(info => {
    if (torBanner) torBanner.classList.toggle("visible", info?.client?.country === "T1");
  });
  ...
  const refs = {
    ...
    scoreLatency:    document.getElementById("score-latency"),
    scoreStability:  document.getElementById("score-stability"),
  ...
  function resetVals() {
    ...
    refs.scoreLatency.textContent = "—";
    refs.scoreStability.textContent = "—";
    refs.scoreReliability.textContent = "—";
    ["scoreLatency","scoreStability","scoreReliability"].forEach(k => refs[k].className = "score-val dim");
  ...
  function setScore(el, score) {
    const labels = { excellent: "Excellent", good: "Good", moderate: "Moderate", poor: "Poor", critical: "Critical", unknown: "—" };
    el.textContent = labels[score] ?? "—";
    el.className = "score-val " + score;
    const card = el.closest(".score-block");
  ...
  function renderJitterChart(varianceMean) {
    ...
    const meanJ = varianceMean ?? (jitterData.reduce((s, d) => s + d[1], 0) / jitterData.length);
    ...
    distChart.setOption({
      ...
      series: [{
        ...
        barMaxWidth: 8,
        itemStyle: {
          color: params => params.value[1] > meanJ * 3 ? C.p90c : C.muted,
  ...
  document.getElementById("providerBtn")?.addEventListener("provider:change", e => {
    selectedProviderId = e.detail;
  });
  ...
  runBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      const providerName = provider?.name ?? "endpoint";
      const baselineProfile = provider?.baselineProfile ?? null;
      const result = await runHealthTest({
        endpoint: provider?.endpoint ?? "/api/ping",
        stabilityInterval: provider?.recommendedIntervalMs ?? 100,
        timeoutMs: provider?.timeoutMs ?? 3000,
        onLatencySample: ({ ttfb }) => {
          if (ttfb == null) return;
          const tSec = parseFloat(((performance.now() - testStart) / 1000).toFixed(3));
          pushLiveSample(ttfb, tSec);
          if (heroRunSub) heroRunSub.textContent = `${liveSamples.length} samples collected`;
        }
      });
      result.provider = { name: providerName, baselineProfile };
      ...
      setVal(refs.jitter, fmtMs(latency?.jitter_median));
      setVal(refs.samples,
      ...
      if (liveSamples.length >= 2) renderJitterChart(latency?.jitter_median);

      const { verdict, findings, scores } = interpret(result);
      lastVerdict = { verdict, findings };
      renderVerdict(result);
      if (advancedToggle) advancedToggle.hidden = false;

      if (scores) {
        setScore(refs.scoreLatency, scores.latency);
        setScore(refs.scoreStability, scores.stability);
        setScore(refs.scoreReliability, scores.reliability);
      }

      const lossPct = latency?.loss != null ? (latency.loss * 100).toFixed(1) + "%" : null;
      setStatus("ok",
        `Done · ${latency?.samples ?? 0}/${latency?.attempts ?? 0} samples · ${(latency?.durationMs / 1000).toFixed(1)}s · route to ${providerName}${lossPct && latency.loss > 0 ? ` · ${lossPct} loss` : ""}`
      );
